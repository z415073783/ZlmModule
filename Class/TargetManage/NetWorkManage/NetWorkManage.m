...
+(void)sendGetRequest:(NSDictionary *)data Trade:(NSString *)trade Port:(int)port Block:(void (^)(id, NSError *))resultBlock
{
    ...
    if (data)
    {
        ...
        for (int i = 0; i<[allKey count]; i++)
        {
            if (i == 0 )
            {
                ...
                [dataStr appendFormat:@"port=%d",port];//提供端口
            }
...
+(void)sendPostRequest:(NSDictionary *)data Trade:(NSString *)trade Port:(int)port Block:(void (^)(id, NSError *))resultBlock
{
    //拼接链接
...
+(void)sendPostFilesRequest:(NSDictionary *)data Files:(NSDictionary *)files Trade:(NSString *)trade Port:(int)port Block:(void (^)(id, NSError *))resultBlock
{
    //拼接链接
    if(!data)
    {
    ...
    [data setValue:[NSString stringWithFormat:@"%d",port] forKey:@"port"];//添加端口
